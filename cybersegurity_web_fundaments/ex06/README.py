om certeza. Foi um processo complexo que envolveu não apenas a exploração de uma vulnerabilidade, mas também a auditoria e correção do próprio ambiente do desafio.

Aqui está um resumo detalhado de todo o processo, formatado para um ficheiro `README.md`.

-----

# Relatório de Exploração: Desafio Cybersecurity 2x2 - Padding Oracle

[cite\_start]Este documento detalha o processo de análise e exploração da vulnerabilidade de *Padding Oracle* presente no desafio "Cybersecurity 2x2"[cite: 2]. [cite\_start]O objetivo final era obter acesso administrativo à aplicação web para capturar a flag[cite: 76, 81].
...
A captura da flag foi bem-sucedida após a inserção dos cookies gerados no navegador. O desafio provou ser mais complexo do que um simples ataque de *Padding Oracle*, exigindo também a identificação e correção de múltiplas falhas de lógica na própria aplicação. O processo demonstrou a importância da auditoria de código e da compreensão profunda dos mecanismos de ataque para contornar ambientes de desafio imperfeitos.
